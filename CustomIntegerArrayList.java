...
public class CustomIntegerArrayList {
	 ...
	 * @return false if the given index is out of range, otherwise true
	 */
	...
	public void remove(int num, int element) {
        ...
        if(num<=0){
            ...
            while(num>0 && arr.contains(element)) {
                arr.remove(Integer.valueOf(element)); //NB : .remove does not work on primitive (int here) so need to call Wrapper Integer
	...
	public ArrayList<Integer> splice(int index, int num) {
		// TODO Implement method
        if(num<=0 || index>=arr.size() || index<0){
            return new ArrayList<Integer>();
        } else {
            ArrayList<Integer> arrRemoved = new ArrayList<Integer>();
            while(num>0 && index<arr.size()){
                arrRemoved.add(arr.remove(index));
                num --;
            } return arrRemoved;
        }
	}
